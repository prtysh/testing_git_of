...
//    finder.setup("haarcascade_frontalface_default.xml");
    ...
    threadedFinder.setup( vidGrabber.getWidth(), vidGrabber.getHeight() );
    threadedFinder.startThread(true);
    ...
    ofBackground(ofColor::black);
        ...
        for(count=0; count < 9; count++) {
            VProjClass * newVideoClass = new VProjClass();
            vp.push_back( newVideoClass  );
...
void ofApp::update()
{
    ofSetWindowTitle(ofToString(ofGetFrameRate(), 2) + " FPS :: " + areaName + " :: " + (this->useBeginEnd ? "begin()/end()" : "draw()"));
        for(count=0; count < vp.size(); count++) {
            vp[count]->vid.update();
            /*-- only grab pixels if new frame! --*/
            if (vp[count]->vid.isFrameNew()) {
                this->vp[count]->tex.loadData(vp[count]->vid.getPixels());
            }
        }
    ...
    for(count = 0; count < vp.size(); count++) {
        if(vp[count]->change < (int)ofGetElapsedTimef()){
            vp[count]->vid.close();
            int idx = ofRandom(0, linesOfTheFile.size() );
            string filename = "CLIPS/" + linesOfTheFile[idx];
            vp[count]->vid.load(filename);
            vp[count]->vid.play();
    ...
    if(vidGrabber.isFrameNew()){
        ...
        threadedFinder.lock();
        threadedFinder.image = color;
        threadedFinder.isFrameNew = true;
        threadedFinder.unlock();
    }
}
...
void ofApp::draw()
{
    ...
    threadedFinder.lock();
    receivedBlobs = threadedFinder.blobs;
    threadedFinder.unlock();

    vidGrabber.draw(0,0);
    for(int i = 0 ; i < receivedBlobs.size(); i++){
        ofSetColor(255);
        ofNoFill();
        ofDrawRectangle(receivedBlobs[i].boundingRect);
        ofFill();
        if (i < vp.size()) {
            if (vp[i]->tex.isAllocated()) {
                vp[i]->tex.draw( receivedBlobs[i].boundingRect );
            }
        }
    }
}
